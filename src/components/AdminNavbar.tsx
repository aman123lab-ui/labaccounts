...
export default function AdminNavbar() {
  ...
  const navItems = [
    ...
    { label: 'Income & Expense', href: '/admin/income-expense' },
  ...
  return (
    <header className="h-16 bg-white border-b border-slate-200 sticky top-0 z-40 backdrop-blur-md w-full flex-shrink-0 print:hidden">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 h-16 flex items-center justify-between gap-2 w-full">
        {/* Logo / Brand Title */}
        <Link href="/admin" className="flex items-center gap-2 flex-shrink-0 z-20 bg-white pr-2 whitespace-nowrap group">
          <div className="w-8 h-8 rounded-lg bg-emerald-600 flex items-center justify-center font-black text-white text-xs shadow-sm flex-shrink-0">
            LA
          </div>
          <span className="font-extrabold text-slate-900 text-xs sm:text-sm tracking-tight group-hover:text-emerald-600 transition-colors whitespace-nowrap flex-shrink-0 inline-flex items-center gap-1">
            <span>Lab Accounting</span>
            <span className="hidden xl:inline text-xs font-mono font-normal text-slate-500 whitespace-nowrap">| Admin</span>
          </span>
        </Link>
            ...
            return (
              ...
              >
                ...
                {Boolean(item.badge && item.badge > 0) && (
                  <span className="bg-emerald-600 text-white font-black text-[9px] sm:text-[10px] font-mono px-1.5 py-0.2 rounded-full shadow-xs flex-shrink-0">
                    {item.badge}
                  </span>
                )}
              </Link>
        ...
        <div className="flex items-center gap-2 flex-shrink-0 z-20 bg-white pl-2 whitespace-nowrap">
          <span className="hidden xl:inline-flex items-center whitespace-nowrap text-[11px] font-mono px-2.5 py-1 rounded-full bg-slate-100 text-slate-700 border border-slate-200 flex-shrink-0">
            Role: Admin
          </span>
          <button
            type="button"
            onClick={handleLogout}
            className="bg-white hover:bg-red-50 text-slate-700 hover:text-red-700 border border-slate-200 hover:border-red-200 text-xs font-semibold px-3 py-1.5 rounded-lg transition-colors whitespace-nowrap flex-shrink-0"
          >
            Log Out
          </button>
        </div>
