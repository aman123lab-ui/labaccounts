  ...
  return (
    <div className="w-full text-xs font-sans">
      ...
      <button
        ...
        className="w-full h-10 bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-left text-slate-100 flex items-center justify-between gap-2 hover:border-slate-700 focus:outline-none focus:border-emerald-500 transition-colors"
      >
        <span className="truncate">
          {selectedAccount ? (
            <span className="flex items-center gap-1.5 truncate">
              <span className="font-semibold text-slate-100 truncate">{selectedAccount.name}</span>
              {selectedAccount.student_name && (
                <span className="text-slate-400 text-[11px] truncate">({selectedAccount.student_name})</span>
              )}
      ...
      {isOpen &&
        ...
        createPortal(
          <div
            ...
            className="bg-slate-900 border border-slate-800 rounded-xl shadow-2xl overflow-hidden max-h-64 flex flex-col font-sans text-xs"
          >
            {/* Search Bar Input */}
            <div className="p-2 border-b border-slate-800 bg-slate-950">
              <input
                ...
                className="w-full bg-slate-900 border border-slate-800 rounded-lg px-3 py-1.5 text-xs text-slate-100 focus:outline-none focus:border-emerald-500 placeholder-slate-500"
              />
            </div>
                  ...
                  return (
                    ...
                    >
                      <div className="truncate min-w-0">
                        ...
                        {acc.student_name && (
                          <span className="block truncate text-[10px] text-slate-400">Student: {acc.student_name}</span>
                        )}
                      </div>
