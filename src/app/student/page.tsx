      ...
      <div className="max-w-4xl mx-auto space-y-8 w-full flex-1">
        ...
        <div className="flex items-start justify-between gap-4 border-b border-slate-800 pb-6">
          <div>
            ...
            <h1 className="text-2xl font-black text-white mt-1">Student Account Dashboard</h1>
          </div>
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {/* Profile Card */}
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 sm:p-5 shadow-xl sm:col-span-2">
                <div className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">
                  Student Profile Details
                </div>
                <h2 className="text-lg font-bold text-slate-100">{studentInfo?.name}</h2>
                <div className="mt-2.5 grid grid-cols-3 gap-3 text-xs font-mono">
                  <div>
                    <span className="text-slate-500 block uppercase">Phone</span>
                    <span className="text-emerald-400 font-semibold">{studentInfo?.phone}</span>
                  </div>
                  <div>
                    <span className="text-slate-500 block uppercase">Batch</span>
                    <span className="text-slate-200 font-semibold">{studentInfo?.batchName}</span>
                  </div>
                  <div>
                    <span className="text-slate-500 block uppercase">Account Type</span>
                    <span className="text-slate-300 font-semibold">Accounts Receivable</span>
                  </div>
                </div>
              </div>

              {/* Outstanding Balance Card */}
              <div className={`border rounded-xl p-4 sm:p-5 shadow-xl flex flex-col justify-between ${
                accountBalance !== null && accountBalance < 0
                  ? 'bg-gradient-to-br from-slate-900 to-amber-950/60 border-amber-800/40'
                  : 'bg-gradient-to-br from-slate-900 to-emerald-950/60 border-emerald-800/40'
              }`}>
                <div>
                  <div className="flex justify-between items-start">
                    <div className={`text-xs font-semibold uppercase tracking-wider mb-1 ${
                      accountBalance !== null && accountBalance < 0 ? 'text-amber-400' : 'text-emerald-400'
                    }`}>
                ...
                <div className="mt-3 space-y-2.5">
                  ...
                  {accountBalance !== null && accountBalance > 0 && (
                    <button
                      ...
                      className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs px-3.5 py-2 rounded-lg shadow-lg transition-all flex items-center justify-center gap-2 group"
                    >
                      <svg className="w-4 h-4 text-emerald-200 group-hover:scale-110 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              {transactions.length === 0 ? (
                ...
                <div className="border border-slate-800 rounded-xl overflow-hidden overflow-x-auto">
                  <table className="w-full text-left text-xs">
                    <thead className="bg-slate-950 text-slate-400 border-b border-slate-800 font-mono uppercase">
                      <tr>
                    ...
                    <tbody className="divide-y divide-slate-800/60 font-mono">
                      {transactions.map((tx) => (
                        <tr key={tx.id} className="hover:bg-slate-800/40">
                          ...
                          <td className="p-3 text-slate-200 font-sans">{tx.description}</td>
