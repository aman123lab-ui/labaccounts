...
async function loadDashboard() {
  ...
  document.getElementById('s-revenue').textContent = fmt(summary.totalRevenue);
  document.getElementById('s-expenses').textContent = fmt(summary.totalExpenses);
  const net = summary.netBalance;
  const netEl = document.getElementById('s-net');
  netEl.textContent = fmt(Math.abs(net));
  netEl.className = `stat-value ${net < 0 ? 'text-danger' : 'text-success'}`;
  document.getElementById('s-debits').textContent = fmt(summary.pendingDebits);
...
window.handleAddRevenue = async function (e) {
  ...
  if (type === 'Cash') {
    const { error } = await addRevenue({
      ...
      notes,
      source: 'Direct Cash',
      description: 'Cash',   // used internally to distinguish from Print; never shown as prefix
    });
    if (error) {
      alertEl.innerHTML = `<div class="alert alert-error">⚠️ ${error}</div>`;
      return;
...
window.handleDebitSubmit = async function (e) {
  ...
  const printTypeVal = typeSelect.value;                       // 'bw' | 'colour'
  ...
  const description = `${pagesVal} pages ${typeLabel} ${sideLabel}`;

  // Build the printMeta object — stored on both the debit ledger entry and the revenue row
  const printMeta = {
    print_type: printTypeVal,   // 'bw' | 'colour'
    print_side: printSideVal,   // 'one' | 'two'
    pages: pagesVal,
  };
